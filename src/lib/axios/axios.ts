...
import { clearAuthToken, getAuthorizationHeader, hasAuthToken, saveAuthToken } from "@/lib/auth/token"
...
const isAuthEndpoint = (url: string): boolean => {
  return url.includes("/auth/login") || url.includes("/auth/password") || url.includes("/auth/signup") || url.includes("/auth/email") || url.includes("/auth/simple-password")
}
 ...
 */
const attemptTokenRefresh = async (apiInstance: AxiosInstance, originalRequest: InternalAxiosRequestConfig): Promise<any> => {
  if (isRefreshing) {
...
api.interceptors.response.use(
  ...
  async (err: AxiosError<ApiResponse>) => {
    ...
    if (statusCode === 401 && hasAuthToken() && !originalRequest._retry && !isAuth) {

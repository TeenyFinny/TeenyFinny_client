...
type VerificationMode = "signup" | "verify" | "update";
...
export default function VerificationForm({
  ...
  // update 모드에서 externalForm이 변경되면 초기값 설정
  useEffect(() => {
    if (mode === "update" && externalForm) {
      if (externalForm.phoneNumber) {
        setPhoneNumber(externalForm.phoneNumber);
      }
      if (externalForm.name) {
        setName(externalForm.name);
      }
    }
  }, [mode, externalForm]);
  ...
  const isButtonEnabled =
    mode === "update"
      ? isBasicInfoComplete
      : isBasicInfoComplete && isOtpInputOpen && isOtpVerified;
  ...
  const handleNext = useCallback(async () => {
    if (mode === "signup") {
      if (!isOtpVerified || !registerSetField) return;
  ...
  const buttonLabel =
    mode === "signup" ? "다음" : mode === "update" ? "저장" : "확인";
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* 인증번호 입력하기 / 재전송 버튼 (update 모드에서는 숨김) */}
      {mode !== "update" && (
          ...
          {isOtpInputOpen ? (
            <>
              ...
              <button
                ...
                className="py-3 px-4 border border-primary-1 rounded-[10px] text-body-02 text-primary-1 bg-white whitespace-nowrap"
              >
                재전송
              </button>
            </>
                ...
                <div className="relative flex-1">
                  <div className="h-[60px] flex items-center">
                    <p className="text-body-04 text-neutral-1 whitespace-pre-line">
                      [Web발신] <br />
                      인증번호를 입력해주세요:{" "}
                      <span className="font-semibold text-netural-1">
                        {displayOtp}
                      </span>
                    </p>
                  </div>
      ...
      >
        {isButtonEnabled ? (
          <BigButtonActivated label={buttonLabel} onClick={handleNext} />
        ) : (

 ...
 */
export function BottomSheetOtp({
  open,
  setOpen,
  onComplete,
  ...
  return (
    <>
      {/* 배경 오버레이 */}
      <div
        className="fixed inset-0 bg-neutral-1/50 z-40"
        onClick={() => setOpen(false)}
      />
      {/* 바텀시트 */}
      <div
        className={`fixed left-0 right-0 bottom-0 bg-white rounded-t-[20px] z-50 ${
          shouldOverlayBottomBar ? "pb-0" : "pb-[56px]"
        }`}
        style={{
      ...
      >
        {/* 드래그 핸들 */}
        <div
          className="w-full h-[30px] flex items-center justify-center pt-2"
        ...
        <div className="px-6 pb-6">
          ...
          />
          {error && (
            <p className="text-error text-body-08 text-center mt-2">{error}</p>
          )}
        </div>

...
/**
 * Step04AuthProps
 * @typedef {Object} Step04AuthProps
 * @property {() => void} onNext - 인증 완료 후 다음 단계로 이동하는 콜백 함수입니다.
 */
 ...
 * - API 호출 중에는 "인증 중..." 메시지를 표시합니다.
 * - 인증 성공 시 초록색 체크 아이콘과 함께 메시지를 표시하고 1초 후 다음 단계로 이동합니다.
 * - 인증 실패 시 빨간색 체크 아이콘과 함께 오류 메시지를 표시합니다.
 *
 * ### 입력 필드
 * - 통신사 선택 (SKT, KT, LG U+ 등)
 * - 휴대폰 번호 (11자리)
 * - 주민등록번호 앞자리 (6자리) + 뒷자리 첫번째 숫자 (1자리)
 * - 이름
 *
 * ### API 연동
 * - POST 요청으로 서버에 인증 정보를 전송합니다.
 * - 응답의 `verified` 필드로 인증 성공 여부를 판단합니다.
 *
 * @component
 * @param {Step04AuthProps} props - Step04Auth 컴포넌트 속성
 * @returns {React.ReactElement} 본인인증 화면 요소
 *
 * @example
 * ```tsx
 * <Step04Auth onNext={() => router.push('/next-step')} />
 * ```
 */
...
export default function Step04Auth({ onNext }: Step04AuthProps) {
  ...
  return (
    <div className="flex flex-col h-full px-[27px]">
      {/* 타이틀 */}
      <div className="mt-[15px] mb-[24px] text-left">

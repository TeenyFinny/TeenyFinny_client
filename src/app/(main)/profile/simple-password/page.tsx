...
export default function SimplePasswordRegisterPage() {
  ...
  const confirmValid = useMemo(() => {
    ...
    return password === confirm && /^[0-9]{6}$/.test(confirm);
  ...
  return (
    <main className="pflex flex-col px-[27px]">
      {/* 제목 */}
      <section className="pt-[36px]">
        <h1 className="text-head-01 text-neutral-1 pb-[10px]">간편 비밀번호 변경</h1>
        <p className="text-body-06 text-neutral-3 whitespace-pre-line">간편 비밀번호는 숫자 6자리 입니다.</p>
      </section>

      {/* 입력 */}
      <section className="flex flex-col gap-[24px] pt-[42px]">
        <div className="flex flex-col">
          <PasswordInput label="간편 비밀번호" value={password} onChange={setPassword} placeholder="6자리 숫자" />
        ...
        <div className="flex flex-col">
          <PasswordInput label="간편 비밀번호 확인" value={confirm} onChange={setConfirm} placeholder="6자리 숫자" />
      ...
      <section className="fixed bottom-[134px] w-full max-w-[327px]">
        {isValid ? <BigButtonActivated label="확인" onClick={handleSubmit} /> : <BigButtonDisabled label="확인" onClick={handleSubmit} />}
      </section>
    </main>
  )
}

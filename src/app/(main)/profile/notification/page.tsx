...
export default function PushSettingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setNightPushEnabled(data?.nightPushEnabled ?? false);
      } catch (err: any) {
  ...
  const handleToggle = async (
    key: "pushEnabled" | "nightPushEnabled",
    value: boolean,
    ...
    try {
      ...
      setter(prev); // 실패 시 롤백
      setMessage(
        err?.response?.data?.message || "알림 설정 변경에 실패했습니다."
      );
    }
  };
    ...
    <main className="w-full h-full max-h-[500px]">
      ...
      <div className="mt-6 flex justify-center">
        <div className="w-[327px] h-[80px] bg-white rounded-xl flex items-center justify-between px-5">
          ...
          <Switch
            checked={pushEnabled}
            onCheckedChange={(v) =>
              handleToggle("pushEnabled", v, setPushEnabled)
            }
          />
        </div>
      </div>
      ...
      <div className="mt-6 flex justify-center">
        <div className="w-[327px] h-[80px] bg-white rounded-xl flex items-center justify-between px-5">
          ...
          <Switch
            checked={nightPushEnabled}
            onCheckedChange={(v) =>
              handleToggle("nightPushEnabled", v, setNightPushEnabled)
            }
          />
        </div>
      </div>

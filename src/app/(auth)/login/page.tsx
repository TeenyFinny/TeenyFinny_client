...
export default function Page() {
  ...
  // 로그인 페이지로 돌아올 때 sessionStorage 전체 삭제 (카카오 로그인 state는 제외)
  useEffect(() => {
    // 카카오 로그인 state는 보존
    const kakaoState = sessionStorage.getItem("kakao-oauth-state");
    ...
    <main className="min-h-[calc(100dvh-44px)]">
      <div className="mx-auto flex w-full flex-col px-6 pb-20 pt-20">
        ...
        <div className="pt-8.5 flex items-center justify-center gap-2">
          <Link
            href="/signup"
            className="text-body-07 text-neutral-3 underline-offset-4 hover:underline"
          >
            회원가입
          </Link>
